classdef FitCircle < handle
    %FitCircle module approximates a given point dataset using single
    ...
    methods (Access = public)
        %% Constructor
        function obj = FitCircle(pts,cov)
    ...
    methods (Access = private)
        ...
        function [res,jac] = createACBlock(obj,A1,A2)
            ac_cov = 1e-4 * eye(4);
            res = obj.InvMahalanobis([A1 - obj.pts(:,1);A2 - obj.pts(:,end)],ac_cov);
            jac = obj.InvMahalanobis([eye(4),zeros(4,1)],ac_cov);
        end
        ...
        function [res,jac] = createMEBlock(obj,A1,A2,k)
            ...
            for i=1:size(obj.pts,2)
                ...
                res(2*i-1:2*i) = obj.InvMahalanobis(me_res,reshape(obj.cov(:,i),2,2));
                jac(2*i-1:2*i,:) = obj.InvMahalanobis(me_jac,reshape(obj.cov(:,i),2,2));
            end
        end
