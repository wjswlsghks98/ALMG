classdef fit2DCircles < handle
    %FIT2DCIRCLES module approximates a given point dataset using multiple
    ...
    methods (Access = public)
        %% Constructor
        function obj = fit2DCircles(varargin)
    ...
    methods (Access = private)
        ...
        function obj = initialize(obj,min_flag)
            oneArc = fit2DCircle(obj.pts,obj.cov);
            oneArc.optimize();
            maxInvalidN = 20;   
            ...
            if valid_flag || obj.arc_num(1) == 1 || min_flag
                ...
                while ub <= length(intvs)
                    ...
                    % disp([lb_idx, ub_idx])
                    sample = fit2DCircle(pts_sample,cov_sample);
                    sample.optimize();
        ...
        function [res,jac] = createACBlock(obj,arcNodes,ks,intv)
            ...
            for i=1:length(idxs)
                if i == 1 || i == length(idxs)
                    ...
                    ac_cov = 10^(2.5) * eye(2);% diag([1e-4,1e-4]); 
                end                
                pt = obj.pts(:,idxs(i));
                arcNode = arcNodes(:,i);
                res(2*i-1:2*i) = InvMahalanobis(obj.static_methods.getACRes(pt,arcNode),ac_cov);
                [I1,J1,V1] = sparseFormat(2*i-1:2*i,2*i-1:2*i,InvMahalanobis(eye(2),ac_cov));
        ...
        function [res,jac] = createMEBlock(obj,arcNodes,ks,intv)
            ...
            for i=1:size(intv,1)
                ...
                for j=lb:ub
                    ...
                    res_seg(2*(j-lb)+1:2*(j-lb+1)) = InvMahalanobis(me_res,me_cov);
                    [I1,J1,V1] = sparseFormat(2*(j-lb)+1:2*(j-lb+1),2*i-1:2*(i+1),InvMahalanobis(aN_jac,me_cov));
                    [I2,J2,V2] = sparseFormat(2*(j-lb)+1:2*(j-lb+1),2*size(arcNodes,2)+i,InvMahalanobis(k_jac,me_cov));
        ...
        function [res,jac] = createCS1Block(obj,arcNodes,ks)
            ...
            if length(ks) > 1
                ...
                for i=2:size(arcNodes,2)-1
                    ...
                    [I1,J1,V1] = sparseFormat(2*(i-2)+1:2*(i-1),2*(i-2)+1:2*(i+1),aN_jac);
                    [I2,J2,V2] = sparseFormat(2*(i-2)+1:2*(i-1),2*size(arcNodes,2)+i-1:2*size(arcNodes,2)+i,k_jac);
        ...
        function [res,jac] = createCS2Block(obj,arcNodes,ks)
            ...
            for i=1:length(ks)
                ...
                [I1,J1,V1] = sparseFormat(i,2*i-1:2*(i+1),aN_jac);
                [I2,J2,V2] = sparseFormat(i,2*size(arcNodes,2)+i,k_jac);
                I(5*(i-1)+1:5*i) = [I1,I2];
                J(5*(i-1)+1:5*i) = [J1,J2];
                V(5*(i-1)+1:5*i) = [V1,V2];
            end
        ...
        function intv = associate(obj,arcNodes)
            ...
            for i=2:size(arcNodes,2)-1
                ...
                % TrueMatchedIntvs = getIntvs(TrueMatchedIdxs);
                ...
                % RelMatchedIdxs = find(d < search_radius);
                RelMatchedIntvs = getIntvs(RelMatchedIdxs);
